...
lblInfo = Label(Tops, font=('Times New Roman', 50, 'bold','underline'), text=" Food Plaza ",fg="Black", bd=10, anchor='w')
lblInfo.grid(row=0, column=0)

lblInfo = Label(Tops, font=('arial', 20, 'bold','underline'), text=localtime, fg="Steel Blue", bd=10, anchor='w')
...
def Ref():
    ...
    CoChilli_Chicken = float(Chilli_Chicken.get()) if Chilli_Chicken.get() else 0
    CoNoodles = float(Noodles.get()) if Noodles.get() else 0
    CoMeals = float(Meals.get()) if Meals.get() else 0
    ...
    CoKadhai_paneer = float(Kadhai_paneer.get()) if Kadhai_paneer.get() else 0
    CoPaneer_chilli = float(Paneer_chilli.get()) if Paneer_chilli.get() else 0
    CoHandi_chicken = float(Handi_chicken.get()) if Handi_chicken.get() else 0
    CoGrill_chicken = float(Grill_chicken.get()) if Grill_chicken.get() else 0
    CoTandoori_roti = float(Tandoori_roti.get()) if Tandoori_roti.get() else 0
    CoPlain_naan = float(Plain_naan.get()) if Plain_naan.get() else 0
    CoButter_naan = float(Butter_naan.get()) if Butter_naan.get() else 0
    CoGarlic_naan = float(Garlic_naan.get()) if Garlic_naan.get() else 0
    CoPlain_rice = float(Plain_rice.get()) if Plain_rice.get() else 0
    CoJeera_rice = float(Jeera_rice.get()) if Jeera_rice.get() else 0
...
btnTotal = Button(f1, padx=16, pady=8, bd=16, fg="black", font=('arial', 16, 'bold'), width=10, text="Total", bg="Sky blue", command=Ref)
btnTotal.grid(row=7, column=2)

btnReset = Button(f1, padx=16, pady=8, bd=16, fg="black", font=('arial', 16, 'bold'), width=10, text="Reset", bg="Sky blue", command=Reset)
btnReset.grid(row=7, column=3)

btnExit = Button(f1, padx=16, pady=8, bd=16, fg="black", font=('arial', 16, 'bold'), width=10, text="Exit", bg="Sky blue", command=qExit)
